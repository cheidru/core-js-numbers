...
function getSumOfDigits(num) {
  ...
  return Math.floor(num % 10) + getSumOfDigits(num / 10);
}
...
function getCountOfOddNumbers(number) {
  let flag = 1;
  if (number < 0) flag = -1;
  if (number * flag === 1) return 1;
  if (number % 2 === 0) return getCountOfOddNumbers(number - 1 * flag);
  return 1 + getCountOfOddNumbers(number - 1 * flag);
}
